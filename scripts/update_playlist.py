...
def check_channel(url, attempts=3):
    ...
    for attempt in range(attempts):
        try:
            ...
            # Tidak perlu mencetak error di sini, cukup coba lagi
            continue
    return False
...
# Gunakan ThreadPoolExecutor untuk memeriksa channel secara paralel
with ThreadPoolExecutor(max_workers=50) as executor:
    ...
    for future in as_completed(future_to_channel):
        ...
        if is_alive:
            ...
            print(f"❌ Channel mati: {channel['url']}")
            dead_count += 1
...
print(f"❌ Channel mati: {dead_count}")
print("==========================")
...
try:
    ...
    print(f"⚠️ Error Git: Gagal melakukan commit atau push. Error: {e}")
