...
# Semaphore = batasi koneksi paralel
semaphore = asyncio.Semaphore(5)

async def fetch(session, url, idx, retries=2):
    """Download dengan retry & timeout"""
    for attempt in range(1, retries + 1):
        try:
            async with semaphore:
                print(f"📡 Mengunduh dari sumber {idx}: {url} (percobaan {attempt})")
            ...
            print(f"⚠️ Error sumber {idx} ({url}) percobaan {attempt}: {e}")
            if attempt == retries:
                return []
    return []
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch(session, url, idx) for idx, url in enumerate(sources, start=1)]
